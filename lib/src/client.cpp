...
namespace konstructs {
    ...
    void Client::open_connection(Settings settings) {
        error_message = "";
        struct hostent *host;
        struct sockaddr_in address;
        if ((host = gethostbyname(settings.server.address.c_str())) == 0) {
            #ifdef _WIN32
            std::cerr << "WSAGetLastError: " << WSAGetLastError() << std::endl;
            #endif
            SHOWERROR("gethostbyname");
            error_message = "Could not find server: " + settings.server.address;
        ...
        version(PROTOCOL_VERSION, settings.server.username, settings.server.password);
    }
